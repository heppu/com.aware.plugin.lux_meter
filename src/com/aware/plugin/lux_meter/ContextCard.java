...
public class ContextCard {
	...
	public static View getContextCard(Context context) {
		...
		if(freq ==  0) {
			context.registerReceiver(card_updater, new IntentFilter(Light.ACTION_AWARE_LIGHT));
		} else {
	...
	public static class CardUpdater extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if(intent.getAction().equals("ACTION_AWARE_LIGHT") && freq == 0) {
				Log.d("asd", "ACTION_AWARE_LIGHT");
				lux_plot.removeAllViews();
				lux_plot.addView(drawGraph(context));
			} else if ((intent.getAction().equals("ACTION_AWARE_PLUGIN_LUX_METER") && freq != 1)){
				Log.d("asd", "ACTION_AWARE_PLUGIN_LUX_METER");
				lux_plot.removeAllViews();
				lux_plot.addView(drawGraph(context));
			}
		}
